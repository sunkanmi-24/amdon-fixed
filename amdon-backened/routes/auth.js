...
console.log(`📧 Attempting to send verification email to ${cleanEmail}, fullName: "${fullName}"`);

sendVerificationEmail({ to: cleanEmail, fullName, code })
  .then(() => {
    console.log(`✅ Verification email sent successfully to ${cleanEmail}`);
  })
  .catch((err) => {
    console.error(`❌ Verification email failed for ${cleanEmail}:`, err.message);
...
router.post(
  ...
  async (req, res) => {
    ...
    const cleanEmail = email.toLowerCase().trim(); // ← ADD THIS

    try {
      const { valid, record } = await verifyCode(cleanEmail, code, 'email_verification'); // ← USE cleanEmail
      ...
      const {data:contactRecord, error:contactError} = await supabaseAdmin
        .from('member_contact')
        .select('member_id')
        .eq('email', cleanEmail) // ← USE cleanEmail
        .single()
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const code = generateOTP();
      await storeCode(cleanEmail, code, 'email_verification'); // ← USE cleanEmail

      await sendVerificationEmail({
        to: cleanEmail, // ← USE cleanEmail
        fullName,
        code,
      });
